...
function UserRolesModal({
  ...
  const [procesandoRol, setProcesandoRol] = useState(null)
  const [mensaje, setMensaje] = useState(null)
  const [userTenantId, setUserTenantId] = useState(null)
  const [rolesUsuario, setRolesUsuario] = useState([])
  const [rolesTenant, setRolesTenant] = useState([])
  const [rolPendienteEliminar, setRolPendienteEliminar] = useState(null)

  const obtenerTenantActual = useCallback(() => {
  ...
  const cargarDatos = useCallback(async () => {
    try {
      ...
      const tenantActual = obtenerTenantActual()

      const relaciones = await obtenerTenantsUsuario(
        usuario.id,
        token
      )
      ...
      const relacion = relaciones.find(
        (item) =>
          Number(item.tenant_id) === tenantActual
      )
      ...
      setUserTenantId(relacion.id)

      const [rolesAsignados, todosLosRoles] =
        await Promise.all([
          obtenerRolesUsuario(relacion.id, token),
          obtenerRoles(token),
        ])

      setRolesUsuario(rolesAsignados || [])
      setRolesTenant(todosLosRoles || [])
    } catch (error) {
      console.error(
  ...
  const idsRolesAsignados = useMemo(() => {
    return new Set(
      rolesUsuario.map(
        (asignacion) => Number(asignacion.role_id)
      )
    )
  }, [rolesUsuario])
  ...
  const rolesDisponibles = useMemo(() => {
    return rolesTenant.filter(
      (rol) => !idsRolesAsignados.has(Number(rol.id))
    )
  }, [rolesTenant, idsRolesAsignados])

  const obtenerInformacionRol = (roleId) => {
    return rolesTenant.find(
      (rol) => Number(rol.id) === Number(roleId)
    )
  }

  const manejarAsignarRol = async (rol) => {
    if (!userTenantId) {
      return
    }

    try {
      setProcesandoRol(`asignar-${rol.id}`)
      setMensaje(null)

      const nuevaAsignacion = await asignarRolUsuario(
        userTenantId,
        rol.id,
        token
      )

      setRolesUsuario((actuales) => [
        ...actuales,
        nuevaAsignacion,
      ])
      ...
      if (error.status === 403) {
        setMensaje({
          tipo: 'danger',
          texto: 'No tienes permisos para asignar roles.',
        })
        return
      }
  ...
  const solicitarEliminarRol = (asignacion) => {
    const rol = obtenerInformacionRol(asignacion.role_id)
  ...
  const confirmarEliminarRol = async () => {
    ...
    const { asignacion, rol } = rolPendienteEliminar

    const nombreRol =
      rol?.name || `Rol ${asignacion.role_id}`

    try {
      setProcesandoRol(`eliminar-${asignacion.id}`)
      setMensaje(null)

      await eliminarRolUsuario(asignacion.id, token)
      ...
      if (error.status === 403) {
        setMensaje({
          tipo: 'danger',
          texto: 'No tienes permisos para retirar roles.',
        })
        return
      }
                    ...
                    <div className="card-body p-2">
                      {rolesUsuario.length === 0 ? (
                        ...
                        <div className="list-group">
                          {rolesUsuario.map((asignacion) => {
                            const rol = obtenerInformacionRol(
                              asignacion.role_id
                            )
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    solicitarEliminarRol(asignacion)
                                  }
                                  disabled={!!procesandoRol}
                                >
                                  Quitar
