    ...
    <Can permission="CLIENT_READ">
      ...
      {abierto && (
        <div className={menuColapsado ? 'd-flex flex-column align-items-center' : 'ps-3'}>
          ...
          <NavLink to={`${rutaTenant}/clientes/cupos`} className={({ isActive }) => menuColapsado ? obtenerClaseCompacta(isActive) : obtenerClaseMenu(isActive)} title="Cupos de crédito" style={({ isActive }) => (isActive ? { backgroundColor: primaryColor } : undefined)}><span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>💳</span>{!menuColapsado && <span className="ms-3">Cupos de crédito</span>}</NavLink>
          ...
          <NavLink to={`${rutaTenant}/clientes/ejecuciones-listas-restrictivas`} className={({ isActive }) => menuColapsado ? obtenerClaseCompacta(isActive) : obtenerClaseMenu(isActive)} title="Ejecuciones listas restrictivas" style={({ isActive }) => (isActive ? { backgroundColor: primaryColor } : undefined)}><span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>⏱️</span>{!menuColapsado && <span className="ms-3">Ejecuciones sincronización</span>}</NavLink>
