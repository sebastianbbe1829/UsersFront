  ...
  return (
    <div>
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-4">
        <div><h2 className="fw-bold mb-1">Tipos de extintores</h2><p className="text-muted mb-0">Catálogo de tipos disponibles para el inventario.</p></div>
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body p-0">
          {cargando ? <div className="text-center py-5"><div className="spinner-border text-primary mb-2" role="status" /><div className="text-muted">Cargando tipos...</div></div> : tipos.length === 0 ? <div className="text-center py-5 text-muted">No hay tipos de extintor registrados.</div> : <>
            <div className="table-responsive d-none d-md-block"><table className="table table-hover align-middle mb-0"><thead><tr><th>Código</th><th>Nombre</th><th>Estado</th><th className="text-end">Acciones</th></tr></thead><tbody>{tipos.map((tipo) => <tr key={tipo.id}><td className="fw-semibold">{tipo.code}</td><td>{tipo.name}</td><td><span className={`badge ${tipo.active ? 'text-bg-success' : 'text-bg-secondary'}`}>{tipo.active ? 'ACTIVO' : 'INACTIVO'}</span></td><td className="text-end"><div className="btn-group btn-group-sm"><button type="button" className="btn btn-outline-primary" onClick={() => abrirEditar(tipo)} disabled={eliminando}>✏️</button><button type="button" className="btn btn-outline-danger" onClick={() => setTipoEliminando(tipo)} disabled={eliminando}>🗑️</button></div></td></tr>)}</tbody></table></div>
            <div className="d-md-none p-3"><div className="d-flex flex-column gap-3">{tipos.map((tipo) => <div key={tipo.id} className="card border shadow-sm"><div className="card-body p-3"><div className="d-flex justify-content-between align-items-start gap-2 mb-3"><div className="min-w-0"><div className="small text-muted">Código</div><div className="fw-bold text-break">{tipo.code}</div></div><span className={`badge flex-shrink-0 ${tipo.active ? 'text-bg-success' : 'text-bg-secondary'}`}>{tipo.active ? 'ACTIVO' : 'INACTIVO'}</span></div><div className="mb-3"><div className="small text-muted">Nombre</div><div className="fw-semibold text-break">{tipo.name}</div></div><div className="d-grid gap-2" style={{ gridTemplateColumns: '1fr 1fr' }}><button type="button" className="btn btn-outline-primary" onClick={() => abrirEditar(tipo)} disabled={eliminando}>✏️ Editar</button><button type="button" className="btn btn-outline-danger" onClick={() => setTipoEliminando(tipo)} disabled={eliminando}>🗑️ Eliminar</button></div></div></div>)}</div></div>
          </>}
        </div>
      </div>
