...
function WelcomePage() {
  ...
  const payload = (() => {
    try { return token ? JSON.parse(atob(token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'))) : null } catch { return null }
  })()
      ...
      <header className="welcome-header">
        <div className="welcome-compact-header">
          ...
          <div className="welcome-user-block">
            <strong>{usuarioLogueado?.name || 'Usuario'}</strong>
            <span>👤 {usuarioLogueado?.dni ? `Número de identificación: ${usuarioLogueado.dni}` : 'Sesión activa'}</span>
          </div>
        </div>
        <div className="welcome-compact-meta">
          <span>🏢 {nombreTenant} · Empresa activa</span>
          <span className="welcome-meta-separator">|</span>
          <span>🟢 Activa</span>
          <span className="welcome-meta-separator">|</span>
          <span>🕘 {hora}</span>
          <span className="welcome-meta-separator">|</span>
          <span>📅 {fecha}</span>
          <button type="button" className="welcome-logout-placeholder" onClick={() => navigate(`/${tenant}/logout`)} aria-label="Cerrar sesión">Cerrar Sesión</button>
        </div>
      </header>
        ...
        <section className="welcome-quick-access" aria-label="Accesos rápidos">
          ...
          <div className="welcome-quick-grid">
            {accesosVisibles.map((acceso) => (
              <button key={acceso.path} type="button" className="welcome-quick-card" style={{ '--welcome-card-accent': acceso.accent }} onClick={() => navigate(`/${tenant}/${acceso.path}`)} aria-label={`Abrir ${acceso.title}`}>
                <div className="welcome-quick-card-top"><div className="welcome-quick-icon" aria-hidden="true">{acceso.icon}</div><span className="welcome-quick-arrow" aria-hidden="true">→</span></div>
                <div><div className="welcome-quick-card-title">{acceso.title}</div><div className="welcome-quick-card-description">{acceso.description}</div></div>
              </button>
            ))}
          </div>
        </section>
      )}
      <p className="welcome-footer" style={{ color: colorSecundario }}>{accesosVisibles.length ? 'Selecciona un acceso directo para comenzar.' : 'Selecciona una opción del menú para comenzar.'}</p>
