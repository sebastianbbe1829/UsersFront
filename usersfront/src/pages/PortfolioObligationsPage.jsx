...
const nombreCliente = (cliente) => (
  cliente?.full_name
  || [cliente?.first_name, cliente?.middle_name, cliente?.last_name, cliente?.second_last_name]
    .filter(Boolean)
    .join(' ')
...
export default function PortfolioObligationsPage() {
  ...
  const cargarClientes = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        ...
        if (items.length < pageSizeClientes) break
        pageClientes += 1
      }
  ...
  const cargar = useCallback(async (filtros = {}) => {
    try {
      ...
      setMensaje({
        tipo: 'danger',
        texto: error.message || 'No fue posible cargar las obligaciones.',
      })
    } finally {
      setCargando(false)
    }
  ...
  const clientesOrdenados = useMemo(
    () => [...clientes].sort((a, b) => (
      nombreCliente(a).toLocaleLowerCase('es-CO').localeCompare(
        nombreCliente(b).toLocaleLowerCase('es-CO'),
        'es-CO',
      )
    )),
    [clientes],
  ...
  const obligacionesVisibles = useMemo(
    () => !obligationSeleccionada
      ? obligaciones
      : obligaciones.filter((item) => String(item.id).toLowerCase() === obligationSeleccionada),
    [obligaciones, obligationSeleccionada],
  )

  const totalPages = Math.max(1, Math.ceil(obligacionesVisibles.length / pageSize))
  const currentPage = Math.min(page, totalPages)
  const visibles = useMemo(
    () => obligacionesVisibles.slice(
      (currentPage - 1) * pageSize,
      currentPage * pageSize,
    ),
    [obligacionesVisibles, currentPage, pageSize],
  )
    ...
    <div>
      ...
      {obligationSeleccionada && (
        <div className="alert alert-primary d-flex justify-content-between align-items-center gap-3" role="status">
          ...
          <button type="button" className="btn btn-sm btn-outline-primary" onClick={() => setSearchParams({}, { replace: true })}>
            Mostrar todas
          </button>
        </div>
      )}
        ...
        <div className="card-body">
          <div className="row g-3 align-items-end">
            <div className="col-lg-5">
              ...
              <select id="obligaciones-cliente" className="form-select" value={clientId} onChange={(event) => void cambiarCliente(event)} disabled={cargandoClientes || cargando}>
            ...
            <div className="col-sm-6 col-lg-2">
              ...
              <input id="obligaciones-desde" type="date" className="form-control" value={dateFrom} onChange={(event) => void cambiarFechaDesde(event)} disabled={cargando} />
            </div>
            <div className="col-sm-6 col-lg-2">
              <label className="form-label fw-semibold" htmlFor="obligaciones-hasta">Hasta</label>
              <input id="obligaciones-hasta" type="date" className="form-control" value={dateTo} onChange={(event) => void cambiarFechaHasta(event)} disabled={cargando} />
            </div>
            <div className="col-lg-3 d-flex justify-content-end">
              <button type="button" className="btn btn-outline-secondary" onClick={() => void limpiarFiltros()} disabled={cargando || (!clientId && !dateFrom && !dateTo && !obligationSeleccionada)}>
                Limpiar
              </button>
            </div>
          </div>
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body border-bottom">
          <div className="d-flex justify-content-end align-items-center">
            <label className="d-flex align-items-center gap-2 mb-0 small text-muted">
              Mostrar
              <select
            ...
            <span className="text-muted small ms-3">{obligacionesVisibles.length} registros</span>
          </div>
        </div>
          ...
          <table className="table table-hover align-middle mb-0">
            ...
            <tbody>
              {cargando && (
                <tr>
                  <td colSpan="7" className="text-center py-5">
                    ...
                    <div className="text-muted mt-2">Consultando obligaciones...</div>
                  </td>
                </tr>
              )}

              {!cargando && !obligacionesVisibles.length && (
                <tr>
                  <td colSpan="7" className="text-center text-muted py-5">
                    {obligationSeleccionada ? 'No se encontró la obligación seleccionada.' : 'No hay obligaciones para los filtros seleccionados.'}
                  </td>
                </tr>
              )}
              ...
              {!cargando && visibles.map((item) => {
                ...
                return (
                  <tr key={item.id} className={obligationSeleccionada === String(item.id).toLowerCase() ? 'table-active' : ''}>
                    <td>
                    ...
                    <td>
                      {pagosAsociados.length
                        ? pagosAsociados.map(({ pago, allocation }) => (
                          <button key={allocation.id || `${pago.id}-${allocation.obligation_id}`} type="button" className="btn btn-link btn-sm p-0 d-block text-decoration-none" onClick={() => abrirPago(pago.id)} title="Abrir este pago">
                            {pago.reference || 'Pago'} — {money(allocation.amount)}
                          </button>
                        ))
        ...
        {!cargando && visibles.length > 0 && (
          <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 p-3">
            ...
            <div className="btn-group">
              <button type="button" className="btn btn-outline-secondary btn-sm" disabled={currentPage === 1} onClick={() => setPage((p) => p - 1)}>
                Anterior
              </button>
              <button type="button" className="btn btn-outline-secondary btn-sm" disabled={currentPage === totalPages} onClick={() => setPage((p) => p + 1)}>
                Siguiente
              </button>
            </div>
          </div>
