...
function ClientsPage() {
  ...
  useEffect(() => {
    ...
    obtenerDepartamentosCliente(token, formulario.country_id)
      ...
      .catch((error) => { setDepartamentos([]); if (error.status === 401) manejarSesionExpirada() })
  ...
  useEffect(() => {
    ...
    obtenerCiudadesCliente(token, formulario.department_id)
      ...
      .catch((error) => { setCiudades([]); if (error.status === 401) manejarSesionExpirada() })
  ...
  const cerrarModal = () => { setModalAbierto(false); setClienteEditando(null); setFormulario(formularioInicial); setDepartamentos([]); setCiudades([]) }
  ...
  const editar = (cliente) => { setClienteEditando(cliente); setFormulario({ ...formularioInicial, ...cliente, identification_type_id: cliente.identification_type_id ?? '', country_id: cliente.country_id ?? '', department_id: cliente.department_id ?? '', city_id: cliente.city_id ?? '', consent_given: Boolean(cliente.consent_given), consent_at: cliente.consent_at ?? null, consent_source: cliente.consent_source ?? '' }); setMensaje(null); setModalAbierto(true) }
