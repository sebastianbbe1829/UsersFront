...
const METHODS = ['EFECTIVO', 'TARJETA', 'TRANSFERENCIA', 'PSE', 'OTRO', 'CARTERA']

const mensajeUsuario = (error, fallback) => {
  const raw = String(error?.message || '').trim()
  const creditMatch = raw.match(/(?:Client credit limit exceeded|Insufficient available credit)\.\s*(?:Available credit:\s*)?([\d.,]+)/i)
  if (creditMatch) {
...
export default function SalesCheckoutPage() {
  ...
  const carteraSelected = effectivePayments.some((p) => p.method === 'CARTERA' && Number(p.amount) > 0); const carteraClient = mode === 'client' && participants.length === 1 ? participants[0] : null
  ...
  const changeMode = (nextMode) => { setMode(nextMode); if (nextMode === 'split') setPayments((current) => current.map((p) => p.method === 'CARTERA' ? { ...p, method: 'EFECTIVO' } : p)); if (nextMode === 'generic') setParticipants([]); if (nextMode === 'client' && participants.length > 1) setParticipants((current) => current.slice(0, 1)) }
  ...
  const changePaymentMethod = (i, value) => {
    ...
    if (value === 'CARTERA' && mode !== 'client') setCreditModal(true)
  }
  ...
  const submit = async () => {
    ...
    if (carteraSelected && mode !== 'client') return setMessage({ type: 'warning', text: 'Los pagos por cartera requieren un único cliente registrado.' })
    if (carteraSelected && !carteraClient) return setMessage({ type: 'warning', text: 'Selecciona un cliente registrado para aplicar un pago por cartera.' })
  ...
  return <>
    ...
    {creditModal && <><div className="modal d-block" tabIndex="-1" role="dialog" aria-modal="true" style={{ zIndex: 1060 }}><div className="modal-dialog modal-dialog-centered"><div className="modal-content shadow"><div className="modal-header"><h5 className="modal-title">Venta con cartera</h5><button type="button" className="btn-close" aria-label="Cerrar" onClick={() => setCreditModal(false)} /></div><div className="modal-body"><div className="alert alert-warning mb-0">Pago por cartera: requiere un único cliente registrado. Puedes combinar CARTERA con otras formas de pago; la obligación se genera únicamente por el valor de CARTERA.</div></div><div className="modal-footer"><button type="button" className="btn btn-primary" onClick={() => setCreditModal(false)}>Aceptar</button></div></div></div></div><div className="modal-backdrop show" style={{ zIndex: 1055 }} onClick={() => setCreditModal(false)} /></>}
