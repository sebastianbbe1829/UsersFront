...
export default function SalesCheckoutPage() {
  ...
  const changePaymentMethod = (i, value) => setPayments((p) => p.map((x, n) => n === i ? { ...x, method: value, amount: value === 'CREDITO' && mode === 'client' ? String(Math.round(total)) : x.amount } : x))
  ...
  const submit = async () => {
    ...
    try {
      ...
      const sale = await crearVenta({ items: cart.map((x) => ({ product_id: x.product.id, quantity: x.quantity })), customers, payments: payments.map((p) => ({ payment_method: p.method, amount: Math.round(Number(p.amount) || 0) })), discount_percentage: roundMoney(discountValue) }, token)
