...
export default function ClientCatalogCrudPage({ title, description, loader, columns, createItem, updateItem, deleteItem, formFields = [] }) {
  ...
  const renderField = (field) => {
    if (field.type === 'checkbox') return <div className="form-check mt-2"><input className="form-check-input" type="checkbox" checked={Boolean(form[field.key])} onChange={(e) => setForm((p) => ({ ...p, [field.key]: e.target.checked }))} disabled={saving} /><label className="form-check-label">{field.label}</label></div>
    if (field.type === 'select') return <select className="form-select" value={form[field.key] ?? ''} onChange={(e) => setForm((p) => ({ ...p, [field.key]: e.target.value }))} required={field.required} disabled={saving}><option value="">Seleccione...</option>{(field.options || []).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
