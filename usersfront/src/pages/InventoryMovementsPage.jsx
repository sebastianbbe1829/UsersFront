...
function InventoryMovementsPage() {
  ...
  const devolver = async (item) => { const confirmar = window.confirm(`¿Desea devolver el movimiento de ${number(item.quantity)} unidades?\n\nSe creará automáticamente el movimiento inverso con los mismos valores del movimiento original.`); if (!confirmar) return; setDevolviendo(item.id); setMensaje(null); try { await devolverMovimientoInventario(item.id, token); await cargarMovimientos(page); setMensaje({ tipo: 'success', texto: 'Movimiento devuelto correctamente.' }) } catch (error) { if (error.status === 401) manejarSesionExpirada(); else setMensaje({ tipo: 'danger', texto: error.message || 'No fue posible devolver el movimiento.' }) } finally { setDevolviendo(null) } }
