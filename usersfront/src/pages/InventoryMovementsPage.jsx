...
function InventoryMovementsPage() {
  ...
  return <>
    ...
    <div className="card shadow-sm border-0"><div className="card-body">
      ...
      {!cargando && movimientos.length > 0 && <><div className="table-responsive"><table className="table table-hover align-middle mb-0"><thead><tr><th>Fecha</th><th>Producto</th><th>Tipo</th><th>Operación</th><th className="text-end">Cantidad</th><th className="text-end">Precio compra</th><th className="text-end">Precio venta</th><th className="text-end">Antes</th><th className="text-end">Después</th><th>ID movimiento</th><th>ID movimiento original</th><th>Notas</th><th>Acción</th></tr></thead><tbody>{movimientos.map((item) => { const producto = productoPorId.get(item.product_id); const reversedQuantity = Number(item.reversed_quantity || 0); const movementQuantity = Number(item.quantity || 0); const fullyReversed = item.origin_type !== 'REVERSAL' && reversedQuantity >= movementQuantity; const salePrice = precioVentaMovimiento(item); return <tr key={item.id}><td>{formatFechaColombia(item.created_at)}</td><td><strong>{producto?.code || `#${item.product_id}`}</strong><br /><small>{producto?.name || 'Producto no disponible'}</small></td><td><MovementBadge type={item.movement_type} /></td><td>{ORIGIN_LABELS[item.origin_type] || item.origin_type}</td><td className="text-end">{number(item.quantity)}</td><td className="text-end">{money(item.unit_purchase_price)}</td><td className="text-end">{money(salePrice)}</td><td className="text-end">{number(item.balance_before)}</td><td className="text-end fw-semibold">{number(item.balance_after)}</td><td><small>{item.id}</small></td><td><small>{item.reversal_of_id || '-'}</small></td><td>{item.notes || '-'}</td><td>{item.origin_type === 'REVERSAL' || fullyReversed ? <span className="text-muted">{fullyReversed ? 'Reversado' : '-'}</span> : <Can permission="INVENTORY_MOVEMENT_CREATE"><button type="button" className="btn btn-sm btn-outline-secondary" disabled={devolviendo === item.id} onClick={() => abrirReversion(item)}>{devolviendo === item.id ? 'Devolviendo...' : '↩ Devolver'}</button></Can>}</td></tr> })}</tbody></table></div><Pagination total={page * PAGE_SIZE + (hasNext ? 1 : 0)} page={page} onPageChange={cargarMovimientos} hasNext={hasNext} /></>}
    </div></div>
