...
const emptyMovement = {
  product_id: '', movement_type: 'ENTRY', origin_type: 'PURCHASE', quantity: '',
  unit_purchase_price: '', profit_percentage: '', notes: '',
}
...
function InventoryPage() {
  ...
  const cargarCatalogos = useCallback(async () => {
    try {
      ...
      const [tiposResult, productosResult, inventarioResult] = await Promise.all([
        obtenerTiposInventario(token), obtenerProductosInventario(token), obtenerInventario(token),
      ])
  ...
  const tiposFiltrados = useMemo(() => term
    ? tipos.filter((item) => `${item.code} ${item.name}`.toLowerCase().includes(term)) : tipos, [tipos, term])
  const productosFiltrados = useMemo(() => term
    ? productos.filter((item) => `${item.code} ${item.name} ${tipoPorId.get(item.inventory_type_id)?.name || ''}`.toLowerCase().includes(term))
    : productos, [productos, tipoPorId, term])
  const inventarioFiltrado = useMemo(() => term
    ? inventario.filter((item) => `${productoPorId.get(item.product_id)?.code || ''} ${productoPorId.get(item.product_id)?.name || ''}`.toLowerCase().includes(term))
    : inventario, [inventario, productoPorId, term])
  ...
  const cargarMovimientos = useCallback(async (productId, requestedPage = 1) => {
    if (!productId) {
      setMovimientos([])
      setMovementHasNext(false)
      return
    }
    try {
      setCargandoMovimientos(true)
      const result = await obtenerMovimientosInventario(productId, token, {
        limit: PAGE_SIZE + 1, offset: (requestedPage - 1) * PAGE_SIZE,
      })
      const rows = Array.isArray(result) ? result : []
      setMovimientos(rows.slice(0, PAGE_SIZE))
      setMovementHasNext(rows.length > PAGE_SIZE)
      setMovementPage(requestedPage)
      setMensaje(null)
    } catch (error) {
      if (error.status === 401) return manejarSesionExpirada()
  ...
  const guardarTipo = async (event) => {
    ...
    const ok = editandoTipo
      ? await ejecutar(() => actualizarTipoInventario(editandoTipo, data, token), 'Tipo actualizado correctamente.')
      : await ejecutar(() => crearTipoInventario(data, token), 'Tipo creado correctamente.')
    if (ok) cerrarModal()
  }
  ...
  const guardarProducto = async (event) => {
    ...
    const ok = editandoProducto
      ? await ejecutar(() => actualizarProductoInventario(editandoProducto, data, token), 'Producto actualizado correctamente.')
      : await ejecutar(() => crearProductoInventario(data, token), 'Producto creado correctamente.')
    if (ok) cerrarModal()
  }
  ...
  const guardarMovimiento = async (event) => {
    ...
    if (ok) {
      ...
      setProductoMovimiento(productId)
      cerrarModal()
      await cargarMovimientos(productId, 1)
    }
  }
  ...
  const iniciarEdicionProducto = (item) => {
    ...
    setProductoForm({ code: item.code, name: item.name, inventory_type_id: String(item.inventory_type_id), active: item.active })
    setModal('product')
  }
  ...
  const cambiarTab = (value) => {
    setPage(1)
    setBusqueda('')
    navigate(value === TABS.stock ? '/inventarios' : `/inventarios/${value}`)
  }

  const abrirMovimiento = () => {
    setMovimientoForm({ ...emptyMovement, product_id: productoMovimiento })
    setModal('movement')
  }

  const origenCambia = (value) => {
    setMovimientoForm({
      ...movimientoForm,
      origin_type: value,
      movement_type: value === 'PURCHASE' || value === 'SALES_RETURN' ? 'ENTRY' : 'EXIT',
    })
  }
  ...
  const renderPagination = (total, currentPage, onPageChange, hasNext = false) => {
    ...
    return (
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-3">
        <small className="text-muted">Página {currentPage}{pages > 1 ? ` de ${pages}` : ''}</small>
        <div className="btn-group" role="group" aria-label="Paginación">
          <button type="button" className="btn btn-outline-secondary btn-sm" disabled={currentPage === 1} onClick={() => onPageChange(currentPage - 1)}>Anterior</button>
          <button type="button" className="btn btn-outline-secondary btn-sm" disabled={!hasNext && currentPage >= pages} onClick={() => onPageChange(currentPage + 1)}>Siguiente</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div><h2 className="fw-bold mb-1">Inventarios</h2><p className="text-muted mb-0">Catálogos, existencias y kardex del tenant.</p></div>
        <Can permission="INVENTORY_MOVEMENT_CREATE"><button type="button" className="btn btn-primary" onClick={abrirMovimiento}>+ Registrar movimiento</button></Can>
      </div>
      ...
      <div className="card shadow-sm border-0">
        <div className="card-header bg-transparent border-0 pt-3">
          <div className="row g-2">
            {[[TABS.stock, '📦', 'Inventario', 'Existencias actuales'], [TABS.types, '🏷️', 'Tipos', 'Clasificación'], [TABS.products, '🛒', 'Productos', 'Catálogo'], [TABS.movements, '📋', 'Movimientos', 'Kardex']].map(([value, icon, label, description]) => (
              <div className="col-6 col-xl-3" key={value}>
                <button type="button" className={`btn w-100 text-start border p-3 h-100 ${tab === value ? 'btn-primary' : 'btn-light'}`} onClick={() => cambiarTab(value)}>
                  <span className="fs-5 me-2">{icon}</span><strong>{label}</strong>
                  <small className={`d-block mt-1 ${tab === value ? 'text-white-50' : 'text-muted'}`}>{description}</small>
                </button>
              </div>
            ))}
          </div>
        ...
        <div className="card-body">
          {tab !== TABS.movements && (
            <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
              <div className="input-group" style={{ maxWidth: 420 }}>
                ...
                <input type="search" className="form-control" value={busqueda} onChange={(event) => { setBusqueda(event.target.value); setPage(1) }} placeholder="Buscar por código o nombre..." aria-label="Buscar en inventario" />
              </div>
              {tab === TABS.types && <Can permission="INVENTORY_CREATE"><button type="button" className="btn btn-outline-primary" onClick={() => setModal('type')}>+ Nuevo tipo</button></Can>}
              {tab === TABS.products && <Can permission="INVENTORY_CREATE"><button type="button" className="btn btn-outline-primary" onClick={() => setModal('product')}>+ Nuevo producto</button></Can>}
            </div>
          )}
          ...
          {!cargando && tab === TABS.types && (
            <><TableTypes rows={paginaTipos} onEdit={iniciarEdicionTipo} />{tiposFiltrados.length === 0 && <EmptyState text="No hay tipos de inventario." />}{renderPagination(tiposFiltrados.length, page, setPage)}</>
          )}
          {!cargando && tab === TABS.products && (
            <><TableProducts rows={paginaProductos} tipoPorId={tipoPorId} onEdit={iniciarEdicionProducto} />{productosFiltrados.length === 0 && <EmptyState text="No hay productos." />}{renderPagination(productosFiltrados.length, page, setPage)}</>
          )}
          {!cargando && tab === TABS.stock && (
            <><TableStock rows={paginaInventario} productoPorId={productoPorId} tipoPorId={tipoPorId} />{inventarioFiltrado.length === 0 && <EmptyState text="No hay existencias para mostrar." />}{renderPagination(inventarioFiltrado.length, page, setPage)}</>
          )}
          {!cargando && tab === TABS.movements && (
            <div className="row g-4">
              <div className="col-12">
                <div className="p-3 bg-light rounded border d-flex flex-wrap justify-content-between align-items-center gap-3">
                  <div><h5 className="fw-bold mb-1">Kardex por producto</h5><p className="text-muted mb-0">Consulta el historial de entradas y salidas sin modificar existencias directamente.</p></div>
                  <Can permission="INVENTORY_MOVEMENT_CREATE"><button type="button" className="btn btn-primary" onClick={abrirMovimiento}>Registrar movimiento</button></Can>
                </div>
              </div>
              <div className="col-12">
                <label className="form-label fw-semibold">Producto</label>
                <select className="form-select" value={productoMovimiento} onChange={(e) => { setProductoMovimiento(e.target.value); setMovementPage(1) }}>
                  <option value="">Seleccione producto...</option>
                  {productos.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}
                </select>
              </div>
              <div className="col-12">
                {cargandoMovimientos && <div className="text-center py-4"><div className="spinner-border spinner-border-sm" /> <span className="text-muted ms-2">Cargando kardex...</span></div>}
                {!cargandoMovimientos && !productoMovimiento && <EmptyState text="Seleccione un producto para consultar sus movimientos." />}
                {!cargandoMovimientos && productoMovimiento && movimientos.length === 0 && <EmptyState text="Este producto todavía no tiene movimientos." />}
                {!cargandoMovimientos && movimientos.length > 0 && (
                  <>
                    <div className="table-responsive"><table className="table table-hover align-middle mb-0"><thead><tr><th>Fecha</th><th>Tipo</th><th>Origen</th><th className="text-end">Cantidad</th><th className="text-end">Antes</th><th className="text-end">Después</th><th>Notas</th></tr></thead><tbody>{movimientos.map((item) => <tr key={item.id}><td>{new Date(item.created_at).toLocaleString('es-CO')}</td><td><MovementBadge type={item.movement_type} /></td><td>{item.origin_type}</td><td className="text-end">{number(item.quantity)}</td><td className="text-end">{number(item.balance_before)}</td><td className="text-end fw-semibold">{number(item.balance_after)}</td><td>{item.notes || '-'}</td></tr>)}</tbody></table></div>
                    {renderPagination(movementPage * PAGE_SIZE + (movementHasNext ? 1 : 0), movementPage, (nextPage) => cargarMovimientos(productoMovimiento, nextPage), movementHasNext)}
                  </>
                )}
              </div>
            </div>
      ...
      {modal === 'type' && <TypeModal editando={Boolean(editandoTipo)} form={tipoForm} setForm={setTipoForm} guardando={guardando} onClose={cerrarModal} onSubmit={guardarTipo} />}
      {modal === 'product' && <ProductModal editando={Boolean(editandoProducto)} form={productoForm} setForm={setProductoForm} tipos={tipos} guardando={guardando} onClose={cerrarModal} onSubmit={guardarProducto} />}
      {modal === 'movement' && <MovementModal form={movimientoForm} setForm={setMovimientoForm} productos={productos} guardando={guardando} onClose={cerrarModal} onSubmit={guardarMovimiento} onOriginChange={origenCambia} />}
    </>
  )
}

function TableTypes({ rows, onEdit }) {
...
function TableProducts({ rows, tipoPorId, onEdit }) {
...
function TableStock({ rows, productoPorId, tipoPorId }) {
...
function TypeModal({ editando, form, setForm, guardando, onClose, onSubmit }) {
...
function ProductModal({ editando, form, setForm, tipos, guardando, onClose, onSubmit }) {
