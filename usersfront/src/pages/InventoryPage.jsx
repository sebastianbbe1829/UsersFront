...
function InventoryPage() {
  ...
  useEffect(() => {
    if (!token) return
    const cargar = async () => { await cargarCatalogos() }
    void cargar()
  ...
  const tiposFiltrados = useMemo(() => term ? tipos.filter((item) => `${item.code} ${item.name}`.toLowerCase().includes(term)) : tipos, [tipos, term])
  const productosFiltrados = useMemo(() => term ? productos.filter((item) => `${item.code} ${item.name} ${tipoPorId.get(item.inventory_type_id)?.name || ''}`.toLowerCase().includes(term)) : productos, [productos, tipoPorId, term])
  const inventarioFiltrado = useMemo(() => term ? inventario.filter((item) => `${productoPorId.get(item.product_id)?.code || ''} ${productoPorId.get(item.product_id)?.name || ''}`.toLowerCase().includes(term)) : inventario, [inventario, productoPorId, term])
  ...
  useEffect(() => {
    if (tab !== TABS.movements || !productoMovimiento) return
    const cargar = async () => { await cargarMovimientos(productoMovimiento, 1) }
    void cargar()
  ...
  const cerrarModal = () => {
    setModal(null); setEditandoTipo(null); setEditandoProducto(null)
    setTipoForm(emptyType); setProductoForm(emptyProduct); setMovimientoForm(emptyMovement)
  }
  ...
  const guardarProducto = async (event) => {
    event.preventDefault()
    const data = { ...productoForm, code: productoForm.code.trim().toUpperCase(), name: productoForm.name.trim(), inventory_type_id: Number(productoForm.inventory_type_id) }
  ...
  const guardarMovimiento = async (event) => {
    event.preventDefault()
    const data = { ...movimientoForm, product_id: Number(movimientoForm.product_id), quantity: Number(movimientoForm.quantity), unit_purchase_price: movimientoForm.unit_purchase_price === '' ? null : Number(movimientoForm.unit_purchase_price), profit_percentage: movimientoForm.profit_percentage === '' ? null : Number(movimientoForm.profit_percentage), notes: movimientoForm.notes.trim() || null }
    const ok = await ejecutar(() => crearMovimientoInventario(data, token), 'Movimiento registrado correctamente.')
    if (ok) {
      const productId = String(data.product_id)
      setProductoMovimiento(productId); cerrarModal(); await cargarMovimientos(productId, 1)
    }
  }
