...
function ExtinguishersPage() {
  ...
  const renderAcciones = (e, mobile = false) => (
    <div className={mobile ? 'd-grid gap-2' : 'd-flex justify-content-center gap-1'} style={mobile ? { gridTemplateColumns: e.active ? 'repeat(4, 1fr)' : 'repeat(3, 1fr)' } : undefined}>
      <button type="button" className="btn btn-outline-primary btn-sm" title="Ver" onClick={() => abrirDetalle(e)}>{mobile ? '👁 Ver' : '👁'}</button>
      <button type="button" className="btn btn-outline-info btn-sm" title="Revisiones" onClick={() => abrirRevisiones(e)}>{mobile ? '📋 Revisiones' : '📋'}</button>
      <button type="button" className="btn btn-outline-warning btn-sm" title="Editar" onClick={() => abrirEditar(e)}>{mobile ? '✏️ Editar' : '✏️'}</button>
      {e.active && <button type="button" className="btn btn-outline-danger btn-sm" title="Desactivar" onClick={() => setEliminandoItem(e)} disabled={eliminando}>{mobile ? '🗑️ Desactivar' : '🗑️'}</button>}
    </div>
  )
  ...
  return <div>
    ...
    {cargando ? <div className="card border-0 shadow-sm"><div className="card-body text-center py-5"><div className="spinner-border text-primary mb-2" role="status" /><div className="text-muted">Cargando extintores...</div></div></div> : <>
      ...
      <div className="d-md-none">
        {visibles.length === 0 ? <div className="card shadow-sm border-0"><div className="card-body text-center py-4 text-muted">No se encontraron extintores.</div></div> : <div className="d-flex flex-column gap-3">{visibles.map((e) => {
          ...
          return <div key={e.id} className="card shadow-sm border-0">
            <div className="card-body p-3">
              <div className="d-flex justify-content-between align-items-start gap-2 mb-3">
                <div className="min-w-0"><div className="fw-bold fs-5 text-break">{e.code}</div><div className="text-muted small text-break">{e.extinguisher_type?.name || 'Tipo no definido'}</div></div>
                {e.active ? <span className="badge text-bg-success flex-shrink-0">Activo</span> : <span className="badge text-bg-secondary flex-shrink-0">Inactivo</span>}
              </div>
              <div className="row g-2 mb-3">
                <div className="col-6"><div className="small text-muted">Capacidad</div><div className="fw-semibold text-break">{e.capacity || '—'}</div></div>
                <div className="col-6"><div className="small text-muted">Ubicación</div><div className="fw-semibold text-break">{e.location || (e.is_stock ? 'En stock' : '—')}</div></div>
                <div className="col-6"><div className="small text-muted">Revisiones</div><div><span className={`badge ${requiere ? 'text-bg-warning' : 'text-bg-primary'}`}>{contador} / 4</span></div></div>
                <div className="col-6"><div className="small text-muted">Próxima recarga</div><div className="fw-semibold text-break">{e.next_recharge_date || '—'}</div>{alerta && alerta.texto !== 'Normal' && <div className="small fw-semibold">{alerta.texto === 'Hoy' ? '🚨' : '⚠️'} {alerta.texto}</div>}</div>
              </div>
              {requiere && <div className="alert alert-warning py-2 px-3 small mb-3">⚠️ <strong>Hidrostática requerida</strong></div>}
