...
function ExtinguishersPage() {
  ...
  const filtrados = useMemo(() => {
    ...
    return extintores.filter((e) => {
      ...
      const stockOk = stockFiltro === 'todos'
  ...
  const abrirEditar = (e) => {
    ...
    setFormulario({
      ...FORM_INICIAL,
      code: e.code || '',
      extinguisher_type_id: e.extinguisher_type_id ? String(e.extinguisher_type_id) : '',
      capacity: e.capacity || '',
      location: e.location || '',
      last_recharge_date: e.last_recharge_date || '',
      next_recharge_date: e.next_recharge_date || '',
      last_hydrostatic_test_date: e.last_hydrostatic_test_date || '',
      next_hydrostatic_test_date: e.next_hydrostatic_test_date || '',
  ...
  const guardar = async (event) => {
    ...
    try {
      ...
      setExtintores((actuales) => editando
        ? actuales.map((x) => x.id === resultado.id ? resultado : x)
        : [...actuales, resultado])
      setMostrarModal(false)
      setEditando(null)
      setFormulario({ ...FORM_INICIAL })
      setMensaje({
        tipo: 'success',
        texto: editando ? 'Extintor actualizado correctamente.' : 'Extintor creado correctamente.',
      })
    } catch (error) {
      if (error.status === 401) return manejarSesionExpirada()
  ...
  const confirmarEliminacion = async () => {
    ...
    try {
      ...
      setExtintores((actuales) => actuales.map((x) => (
        x.id === eliminandoItem.id
          ? { ...x, active: false, status: 'INACTIVE' }
          : x
      )))
  ...
  const ultimaRevision = (id) => revisiones
    .filter((r) => Number(r.extinguisher_id) === Number(id))
    .sort((a, b) => String(b.inspection_date).localeCompare(String(a.inspection_date)))[0]
  ...
  const renderAcciones = (e, mobile = false) => (
    <div className={mobile ? 'd-grid gap-1' : 'd-flex justify-content-center gap-1'}>
      <button
        type="button"
        className={`btn ${mobile ? 'btn-primary' : 'btn-outline-primary'} btn-sm`}
        title="Ver"
      ...
      <button
        type="button"
        className={`btn ${mobile ? 'btn-primary' : 'btn-outline-info'} btn-sm`}
        title="Revisiones"
      ...
      <button
        type="button"
        className={`btn ${mobile ? 'btn-warning' : 'btn-outline-warning'} btn-sm`}
        title="Editar"
      ...
      {e.active && (
        <button
          type="button"
          className={`btn ${mobile ? 'btn-danger' : 'btn-outline-danger'} btn-sm`}
          title="Desactivar"
  ...
  const renderDesktopRow = (e) => {
    ...
    return (
      <tr key={e.id}>
        ...
        <td>{e.extinguisher_type?.name || '—'}</td>
        <td>{e.capacity || '—'}</td>
        <td>{e.location || '—'}</td>
        <td>
          <span className={`badge ${requiere ? 'text-bg-warning' : 'text-bg-primary'}`}>
            {contador} / 4
          </span>
          {requiere && (
        ...
        <td>
          {e.next_recharge_date ? (
            <>
              <span className={`badge ${alerta?.clase || 'text-bg-secondary'}`}>
                {e.next_recharge_date}
              </span>
              {alerta && alerta.texto !== 'Normal' && (
                <div className="small fw-semibold">
                  {alerta.texto === 'Hoy' ? '🚨' : '⚠️'} {alerta.texto}
                </div>
              )}
            </>
          ) : '—'}
  ...
  const renderMobileCard = (e) => {
    ...
    return (
      <div className="card shadow-sm border-0 mb-3 extinguisher-mobile-card" key={e.id}>
        <div className="card-body p-3">
          <div className="d-flex justify-content-between align-items-start gap-2 mb-3">
            <div>
              <div className="text-muted small">Código</div>
              <div className="fw-bold fs-5">{e.code}</div>
            </div>
            {e.active
              ? <span className="badge text-bg-success">Activo</span>
              : <span className="badge text-bg-secondary">Inactivo</span>}
          ...
          <div className="row g-2 mb-2">
            <div className="col-6">
              ...
              <div className="fw-semibold text-break">{e.extinguisher_type?.name || '—'}</div>
            </div>
            <div className="col-6">
              <div className="text-muted small">Capacidad</div>
              <div className="fw-semibold">{e.capacity || '—'}</div>
            </div>
            <div className="col-12">
              <div className="text-muted small">Ubicación</div>
              <div className="text-break">{e.location || '—'}</div>
            </div>
            <div className="col-6">
              <div className="text-muted small">Revisiones</div>
              <span className={`badge ${requiere ? 'text-bg-warning' : 'text-bg-primary'}`}>
                {contador} / 4
              </span>
            </div>
            <div className="col-6">
              ...
              {e.next_recharge_date
                ? <span className={`badge ${alerta?.clase || 'text-bg-secondary'}`}>{e.next_recharge_date}</span>
          ...
          {ultima && (
            <div className="small text-muted border-top pt-2 mb-2">
              Última revisión: <strong>{ultima.inspection_date}</strong> · {ultima.result}
            </div>
          )}
  ...
  return (
    <div>
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-4">
        <div>
          ...
          <p className="text-muted mb-0">
            Inventario, ubicación, recargas, revisiones y pruebas hidrostáticas.
          </p>
        </div>
            ...
            <div className="card-body py-2 px-3">
              <div className="row g-2 align-items-end">
                <div className="col-12 col-md-5">
                  <label className="form-label fw-semibold small mb-1">Buscar extintor</label>
                  <input
                ...
                <div className="col-6 col-md-2">
                  <label className="form-label fw-semibold small mb-1">Mostrar</label>
                  <select
                    className="form-select form-select-sm"
                    value={porPagina}
                    onChange={(e) => {
          ...
          <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mt-3">
            <div>
              <small className="text-muted">
                Mostrando {desde} - {hasta} de {filtrados.length} extintores
              </small>
            ...
            <div className="d-flex gap-1 flex-wrap">
              {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((p) => (
                <button
                  key={p}
                  type="button"
                  className={`btn btn-sm ${p === paginaSegura ? 'btn-primary' : 'btn-outline-secondary'}`}
                  onClick={() => setPagina(p)}
                >
                  {p}
                </button>
      ...
      {mostrarModal && (
        <Modal
          title={editando ? 'Editar extintor' : 'Nuevo extintor'}
          onClose={cerrarModal}
          footer={(
            <>
              ...
              <button type="submit" form="extinguisher-form" className="btn btn-primary" disabled={guardando}>
                {guardando ? 'Guardando...' : 'Guardar'}
              </button>
            </>
          )}
        >
          <form id="extinguisher-form" onSubmit={guardar}>
            <div className="row g-3">
              <div className="col-12 col-md-6">
                <label className="form-label">Código</label>
                <input className="form-control" name="code" value={formulario.code} onChange={cambiarCampo} required />
              </div>
              <div className="col-12 col-md-6">
                <label className="form-label">Tipo</label>
                <select className="form-select" name="extinguisher_type_id" value={formulario.extinguisher_type_id} onChange={cambiarCampo} required>
              ...
              <div className="col-12 col-md-6">
                ...
                <input className="form-control" name="capacity" value={formulario.capacity} onChange={cambiarCampo} />
              </div>
              <div className="col-12 col-md-6">
                <label className="form-label">Ubicación</label>
                <input className="form-control" name="location" value={formulario.location} onChange={cambiarCampo} />
              </div>
              <div className="col-12 col-md-6">
                <label className="form-label">Última recarga</label>
                <input type="date" className="form-control" name="last_recharge_date" value={formulario.last_recharge_date} onChange={cambiarCampo} />
              </div>
              <div className="col-12 col-md-6">
                <label className="form-label">Próxima recarga</label>
                <input type="date" className="form-control" name="next_recharge_date" value={formulario.next_recharge_date} onChange={cambiarCampo} />
              </div>
              <div className="col-12 col-md-6">
                <label className="form-label">Última prueba hidrostática</label>
                <input type="date" className="form-control" name="last_hydrostatic_test_date" value={formulario.last_hydrostatic_test_date} onChange={cambiarCampo} />
              </div>
              <div className="col-12 col-md-6">
                <label className="form-label">Próxima prueba hidrostática</label>
                <input type="date" className="form-control" name="next_hydrostatic_test_date" value={formulario.next_hydrostatic_test_date} onChange={cambiarCampo} />
              </div>
              <div className="col-12">
                <div className="form-check">
                  <input className="form-check-input" type="checkbox" name="is_stock" id="is_stock" checked={formulario.is_stock} onChange={cambiarCampo} />
                  <label className="form-check-label" htmlFor="is_stock">Este extintor está en stock</label>
                </div>
              </div>
            </div>
          </form>
      ...
      {eliminandoItem && (
        <Modal
          ...
          onClose={() => !eliminando && setEliminandoItem(null)}
          footer={(
            <>
              <button type="button" className="btn btn-secondary" onClick={() => setEliminandoItem(null)} disabled={eliminando}>
                Cancelar
              </button>
        ...
        >
          <p className="mb-0">
            ¿Deseas desactivar el extintor <strong>{eliminandoItem.code}</strong>?
          </p>
        </Modal>
      )}

      {verItem && (
        <Modal title={`Extintor ${verItem.code}`} onClose={() => setVerItem(null)}>
          <div className="row g-3">
            <div className="col-12 col-md-6">
              <div className="text-muted small">Tipo</div>
              <div className="fw-semibold">{verItem.extinguisher_type?.name || '—'}</div>
            </div>
            <div className="col-12 col-md-6">
              <div className="text-muted small">Capacidad</div>
              <div>{verItem.capacity || '—'}</div>
            </div>
            <div className="col-12">
              <div className="text-muted small">Ubicación</div>
              <div>{verItem.location || '—'}</div>
            </div>
            <div className="col-12 col-md-6">
              <div className="text-muted small">Última recarga</div>
              <div>{verItem.last_recharge_date || '—'}</div>
            </div>
            <div className="col-12 col-md-6">
              <div className="text-muted small">Próxima recarga</div>
              <div>{verItem.next_recharge_date || '—'}</div>
            </div>
            <div className="col-12 col-md-6">
              <div className="text-muted small">Última prueba hidrostática</div>
              <div>{verItem.last_hydrostatic_test_date || '—'}</div>
            </div>
            <div className="col-12 col-md-6">
              <div className="text-muted small">Próxima prueba hidrostática</div>
              <div>{verItem.next_hydrostatic_test_date || '—'}</div>
            </div>
          </div>
        </Modal>
      )}
