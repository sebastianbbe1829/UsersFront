...
function ExtinguishersPage() {
  ...
  useEffect(() => { cargarExtintores() }, [cargarExtintores])
  ...
  const guardar = async (event) => {
    ...
    try {
      setGuardando(true)
      const resultado = editando ? await actualizarExtintor(editando, prepararDatos(formulario), token) : await crearExtintor(prepararDatos(formulario), token)
          ...
          <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '700px', width: 'calc(100% - 2rem)', margin: '1rem auto' }}>
            <div className="modal-content" style={{ maxHeight: 'calc(100vh - 2rem)' }}>
              <div className="modal-header py-2 px-3">
                <h5 className="modal-title mb-0">{editando ? 'Editar extintor' : 'Nuevo extintor'}</h5>
                <button type="button" className="btn-close" onClick={cancelar} disabled={guardando} />
              </div>
              ...
              <form onSubmit={guardar} autoComplete="off">
                  ...
                  <div className="row g-3">
                    {campos.map(([campo, etiqueta, tipo]) => (
                      <div className="col-md-6" key={campo}>
                        ...
                        <input className="form-control" name={campo} type={tipo} value={formulario[campo] || ''} onChange={cambiarCampo} required={['code', 'extinguisher_type'].includes(campo)} disabled={guardando} />
                      </div>
                    ))}
                    <div className="col-md-6">
                      <label className="form-label mb-1">Estado</label>
                      <select className="form-select" name="status" value={formulario.status} onChange={cambiarCampo} disabled={guardando}>
                    ...
                    <div className="col-md-6 d-flex align-items-end">
                      <div className="form-check mb-2">
                        <input className="form-check-input" id="is_stock" name="is_stock" type="checkbox" checked={formulario.is_stock} onChange={cambiarCampo} disabled={guardando} />
                ...
                <div className="modal-footer py-2 px-3">
                  <button type="button" className="btn btn-secondary" onClick={cancelar} disabled={guardando}>Cancelar</button>
                  <button type="submit" className="btn btn-primary" disabled={guardando}>
                    {guardando ? <><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true" />Guardando...</> : (editando ? 'Guardar cambios' : 'Crear extintor')}
                  </button>
                </div>
              </form>
            </div>
