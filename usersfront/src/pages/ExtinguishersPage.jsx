...
function ExtinguishersPage() {
  ...
  const renderAcciones = (e, mobile = false) => (
    <div className={mobile ? 'd-grid gap-1' : 'd-flex justify-content-center gap-1'}>
      ...
      {e.active && <button type="button" className={`btn ${mobile ? 'btn-danger' : 'btn-outline-danger'} btn-sm`} title="Desactivar" onClick={() => setEliminandoItem(e)} disabled={eliminando}>{mobile ? '🗑️ Desactivar' : '🗑️'}</button>}
    </div>
  )
  ...
  return (
    <div>
      ...
      {cargando ? (
        <div className="card border-0 shadow-sm"><div className="card-body text-center py-5"><div className="spinner-border text-primary mb-2" role="status" /><div className="text-muted">Cargando extintores...</div></div></div>
      ) : (
        <>
      ...
      {mostrarModal && <Modal title={editando ? 'Editar extintor' : 'Nuevo extintor'} onClose={cerrarModal} footer={<><button type="button" className="btn btn-secondary" onClick={cerrarModal} disabled={guardando}>Cancelar</button><button type="submit" form="extinguisher-form" className="btn btn-primary" disabled={guardando}>{guardando ? 'Guardando...' : 'Guardar'}</button></>}><form id="extinguisher-form" onSubmit={guardar}><div className="row g-3"><div className="col-12 col-md-6"><label className="form-label">Código</label><input className="form-control" name="code" value={formulario.code} onChange={cambiarCampo} required /></div><div className="col-12 col-md-6"><label className="form-label">Tipo</label><select className="form-select" name="extinguisher_type_id" value={formulario.extinguisher_type_id} onChange={cambiarCampo} required><option value="">Selecciona un tipo...</option>{tipos.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div><div className="col-12 col-md-6"><label className="form-label">Capacidad</label><input className="form-control" name="capacity" value={formulario.capacity} onChange={cambiarCampo} /></div><div className="col-12 col-md-6"><label className="form-label">Ubicación</label><input className="form-control" name="location" value={formulario.location} onChange={cambiarCampo} /></div><div className="col-12 col-md-6"><label className="form-label">Última recarga</label><input type="date" className="form-control" name="last_recharge_date" value={formulario.last_recharge_date} onChange={cambiarCampo} /></div><div className="col-12 col-md-6"><label className="form-label">Próxima recarga</label><input type="date" className="form-control" name="next_recharge_date" value={formulario.next_recharge_date} onChange={cambiarCampo} /></div><div className="col-12 col-md-6"><label className="form-label">Última prueba hidrostática</label><input type="date" className="form-control" name="last_hydrostatic_test_date" value={formulario.last_hydrostatic_test_date} onChange={cambiarCampo} /></div><div className="col-12 col-md-6"><label className="form-label">Próxima prueba hidrostática</label><input type="date" className="form-control" name="next_hydrostatic_test_date" value={formulario.next_hydrostatic_test_date} onChange={cambiarCampo} /></div><div className="col-12"><div className="form-check"><input className="form-check-input" type="checkbox" name="is_stock" id="is_stock" checked={formulario.is_stock} onChange={cambiarCampo} /><label className="form-check-label" htmlFor="is_stock">Este extintor está en stock</label></div></div></div></form></Modal>}
