...
const STYLE = `
.sales-pos{height:calc(100vh - 190px);min-height:560px;overflow:hidden}.sales-pos-col{height:100%;overflow:auto;min-width:0}.sales-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem}.sales-product{border:1px solid var(--bs-border-color);background:var(--bs-body-bg);border-radius:.65rem;overflow:hidden;cursor:pointer;text-align:left}.sales-product:hover:not(:disabled){box-shadow:0 .35rem .9rem rgba(0,0,0,.12);transform:translateY(-1px)}.sales-img{height:90px;width:100%;object-fit:cover}.sales-placeholder{height:90px;display:flex;align-items:center;justify-content:center;background:var(--bs-tertiary-bg);font-size:32px}.sales-tabs{display:none}@media(max-width:991.98px){.sales-pos{height:auto;min-height:0;overflow:visible}.sales-pos-col{height:auto;overflow:visible}.sales-tabs{display:flex;position:sticky;top:0;z-index:20;background:var(--bs-body-bg);padding:.45rem 0}.sales-panel{display:none}.sales-panel.active{display:block}.sales-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:575.98px){.sales-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.sales-img,.sales-placeholder{height:82px}}
`
...
export default function SalesPOSPage() {
  ...
  const shownProducts = useMemo(() => { const term = search.trim().toLowerCase(); return products.filter((p) => !term || `${p.code || ''} ${p.name || ''}`.toLowerCase().includes(term)) }, [products, search])
  ...
  const reset = () => { setCart([]); setSearch(''); setMode('generic'); setParticipants([]); setClientSearch(''); setDiscount('0'); setPayments([{ method: 'EFECTIVO', amount: '' }]); setAutoconsumption(false); setActiveDraftId(null); setLastSale(null) }
  ...
  const submit = async () => {
    ...
    try {
      ...
      setLastSale(sale); setMessage({ type: 'success', text: `Venta ${sale.sale_number} realizada correctamente por ${money(sale.total)}.` }); reset(); await Promise.all([loadCatalog(), loadFrozen()])
