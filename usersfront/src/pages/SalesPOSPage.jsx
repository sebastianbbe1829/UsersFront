...
export default function SalesPOSPage() {
  ...
  return <>
    ...
    <div className="container-fluid py-2">
      <div className="d-flex justify-content-between align-items-center mb-2"><div><button className="btn btn-sm btn-link text-decoration-none p-0" onClick={() => navigate('/welcome')}>← Volver</button><h4 className="mb-0">Punto de venta</h4><div className="small text-muted">Registra ventas, pagos y facturación.</div></div><div className="d-flex gap-2"><button className="btn btn-sm btn-outline-secondary" disabled={freezing || !cart.length} onClick={() => void freeze()}>{freezing ? 'Congelando...' : '⏸️ Congelar'}</button><button className="btn btn-sm btn-outline-danger" onClick={() => reset()}>Limpiar</button></div></div>
      {message && <div className={`alert alert-${message.type} py-2`}>{message.text}</div>}
      <div className="sales-tabs"><button className={`btn btn-sm flex-fill ${tab === 'products' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setTab('products')}>Productos</button><button className={`btn btn-sm flex-fill ${tab === 'sale' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setTab('sale')}>Venta</button><button className={`btn btn-sm flex-fill ${tab === 'frozen' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setTab('frozen')}>Congeladas ({frozen.length})</button></div>
      <div className="sales-pos">
        <div className={`sales-pos-col sales-panel ${tab === 'products' ? 'active' : ''}`}>
        ...
        <div className={`sales-pos-col sales-panel ${tab === 'sale' ? 'active' : ''}`}>
          {customerBlock}
