  ...
  return <div className={modoOscuro ? 'bg-dark text-light min-vh-100' : 'bg-light min-vh-100'} style={{ display: 'flex' }}>
    <aside className={modoOscuro ? 'bg-black text-light shadow' : 'bg-dark text-white shadow'} style={{ width: menuColapsado ? '72px' : '250px', minHeight: '100vh', transition: 'width .25s ease', position: 'fixed', left: 0, top: 0, bottom: 0, zIndex: 1000, overflow: menuColapsado ? 'visible' : 'hidden' }}>
      ...
      <nav className="d-flex flex-column">
        ...
        {administracionAbierta && menuColapsado && <div className="border-top border-secondary border-bottom border-secondary" style={{ position: 'fixed', left: '72px', top: `${menuColapsadoTopAdministracion}px`, width: '190px', zIndex: 1100, boxShadow: '0 .25rem .75rem rgba(0,0,0,.25)' }}><NavLink to="usuarios" className={({ isActive }) => obtenerClaseMenu(isActive)} title="Usuarios"><span style={{ fontSize: '20px', minWidth: '24px', textAlign: 'center' }}>👥</span><span className="ms-3">Usuarios</span></NavLink><NavLink to="roles" className={({ isActive }) => obtenerClaseMenu(isActive)} title="Roles"><span style={{ fontSize: '20px', minWidth: '24px', textAlign: 'center' }}>🛡️</span><span className="ms-3">Roles</span></NavLink><NavLink to="permisos" className={({ isActive }) => obtenerClaseMenu(isActive)} title="Permisos"><span style={{ fontSize: '20px', minWidth: '24px', textAlign: 'center' }}>🔐</span><span className="ms-3">Permisos</span></NavLink></div>}
