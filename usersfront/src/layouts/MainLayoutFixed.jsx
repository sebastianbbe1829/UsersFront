  ...
  return (
    <div className={modoOscuro ? 'bg-dark text-light min-vh-100' : 'bg-light min-vh-100'} style={{ display: 'flex' }}>
      ...
      <aside
        ...
        style={{
          width: menuColapsado ? '72px' : '250px',
          minHeight: '100vh',
          ...
          bottom: 0,
          zIndex: 1000,
          overflow: menuColapsado ? 'visible' : 'hidden',
        }}
      >
        ...
        <nav className="d-flex flex-column">
          <NavLink
            ...
            className={({ isActive }) => obtenerClaseMenu(isActive)}
            title="Inicio"
          ...
          <Can permission="EXTINGUISHER_READ">
            <button
              type="button"
              className={obtenerClaseMenu(extintoresPorRuta)}
              onClick={() => setExtintoresAbiertos((valor) => !valor)}
              title="Extintores"
            ...
            >
              ...
              {!menuColapsado && (
                <>
                  <span className="ms-3 flex-grow-1 text-start">Extintores</span>
                  <span>{extintoresMenuAbierto ? '▾' : '▸'}</span>
                </>
              )}
            </button>

            {extintoresMenuAbierto && !menuColapsado && (
              <div className="ps-3">
                <Can permission="EXTINGUISHER_READ">
                  <NavLink to={`${rutaTenant}/extintores`} end className={({ isActive }) => obtenerClaseMenu(isActive)} title="Inventario">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>🧯</span>
                    <span className="ms-3">Inventario</span>
                  </NavLink>
                </Can>
                <Can permission="EXTINGUISHER_READ">
                  <NavLink to={`${rutaTenant}/extintores/tipos`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Tipos de extintores">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>🏷️</span>
                    <span className="ms-3">Tipos de extintores</span>
                  </NavLink>
                </Can>
                <Can permission="EXTINGUISHER_READ">
                  <NavLink to={`${rutaTenant}/extintores/revisiones`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Revisiones">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>📋</span>
                    <span className="ms-3">Revisiones</span>
                  </NavLink>
                </Can>
                <Can permission="EXTINGUISHER_READ">
                  <NavLink to={`${rutaTenant}/extintores/items-revision`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Ítems de revisión">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>☑️</span>
                    <span className="ms-3">Ítems de revisión</span>
                  </NavLink>
                </Can>
              </div>
            )}
          ...
          <Can permissions={['USER_READ', 'ROLE_READ', 'PERMISSION_READ']}>
            <button
              type="button"
              className={obtenerClaseMenu(administracionPorRuta)}
            ...
            >
              ...
              {!menuColapsado && (
                <>
                  ...
                  <span>{administracionMenuAbierto ? '▾' : '▸'}</span>
                </>
              )}
            </button>

            {administracionMenuAbierto && !menuColapsado && (
              <div className="ps-3">
                <Can permission="USER_READ">
                  <NavLink to={`${rutaTenant}/usuarios`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Usuarios">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>👥</span>
                    <span className="ms-3">Usuarios</span>
                  </NavLink>
                </Can>
                <Can permission="ROLE_READ">
                  <NavLink to={`${rutaTenant}/roles`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Roles">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>🛡️</span>
                    <span className="ms-3">Roles</span>
                  </NavLink>
                </Can>
                <Can permission="PERMISSION_READ">
                  <NavLink to={`${rutaTenant}/permisos`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Permisos">
                    <span style={{ fontSize: '19px', minWidth: '24px', textAlign: 'center' }}>🔐</span>
          ...
          {esSuper && (
            <NavLink to={`${rutaTenant}/configuracion-ui`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Configuración de la interfaz">
              <span style={{ fontSize: '21px', minWidth: '24px', textAlign: 'center' }}>🎨</span>
          ...
          {esSuper && (
            <NavLink to={`${rutaTenant}/administracion-tenant`} className={({ isActive }) => obtenerClaseMenu(isActive)} title="Administración del tenant">
              <span style={{ fontSize: '21px', minWidth: '24px', textAlign: 'center' }}>🏢</span>
          ...
          {esSuper && (
            <NavLink
              to={`${rutaTenant}/usuarios-super`}
              className={({ isActive }) => obtenerClaseMenu(isActive)}
              title="Usuarios SUPER"
              style={({ isActive }) => (isActive ? { backgroundColor: primaryColor } : undefined)}
            >
              <span style={{ fontSize: '21px', minWidth: '24px', textAlign: 'center' }}>👑</span>
              {!menuColapsado && <span className="ms-3">Usuarios SUPER</span>}
            </NavLink>
          )}
        </nav>
        ...
        <div className="position-absolute bottom-0 start-0 end-0">
          <button
            type="button"
            className="btn text-white border-0 rounded-0 w-100 text-start py-3 px-3"
          ...
          <button
            type="button"
            className="btn text-white rounded-0 w-100 text-start py-3 px-3"
            ...
            {usuarioLogueado && (
              <div className="d-flex align-items-center gap-2">
                ...
                <div className="d-none d-sm-block text-end">
                  ...
                  {esSuper && (
                    <small className="fw-semibold" style={{ color: primaryColor }}>
                      SUPER
                    </small>
                  )}
                </div>
